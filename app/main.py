...
with st.sidebar:
    ...
    if spotify_zip:
        with st.spinner("Processing data..."):
            ...
            st.success(f"{len(spotify_df):,} records loaded")

    # Stop early if no database exists yet — nothing to show on the main page.
    if not DB_PATH.exists():
        st.info("Upload your Spotify ZIP to get started.")
        st.stop()
    ...
    if period_type == "Monthly":
        ...
        date_from = st.date_input("Start", value=min_date, min_value=min_date, max_value=max_date)
        date_to = st.date_input("End", value=max_date, min_value=min_date, max_value=max_date)
...
if overview.empty or int(overview.iloc[0]["total_plays"]) == 0:
    st.warning("No listening records found for this period.")
    st.stop()
...
col1.metric("Total plays",     f"{int(row['total_plays']):,}")
col2.metric("Listening time",  f"{int(row['total_minutes'] / 60)} hrs")
col3.metric("Unique artists",  f"{int(row['unique_artists'])}")
col4.metric("Unique tracks",   f"{int(row['unique_tracks'])}")
...
with left:
    ...
    if not top_artists.empty:
        fig = px.bar(
            ...
            labels={"plays": "Plays", "artist": "", "minutes": "Minutes"},
        )
        fig.update_layout(
...
with right2:
    ...
    fig4 = px.line(
        ...
        labels={"year_month": "Month", "total_minutes": "Total minutes"},
        markers=True,
    )
    if period_type == "Monthly":
...
try:
    ...
    if tables:
        ...
        film_tab1, film_tab2 = st.tabs(["📔 Diary", "👁 All Watched"])
            ...
            if "diary" in tables:
                ...
                if not diary_df.empty:
                    diary_df = diary_df.rename(columns={
                        "name": "Film", "year": "Year", "rating": "Rating",
                        "rewatch": "Rewatch", "watched_date": "Watch Date",
                    })
                    if tmdb_api_key:
                        with st.spinner("Loading TMDB data..."):
                            enrich_df(diary_df, tmdb_api_key, "Film", "Year")
                    st.dataframe(diary_df, use_container_width=True, hide_index=True)
                else:
                    st.caption("No diary entries for this period.")
            else:
                st.caption("No diary data loaded.")
            ...
            if "watched_list" in tables:
                ...
                if not watched_df.empty:
                    ...
                    watched_df = watched_df.rename(columns={
                        "name": "Film", "year": "Year",
                        "date": "Date", "in_diary": "In Diary",
                    })
                    if tmdb_api_key:
                        with st.spinner("Loading TMDB data..."):
                            enrich_df(watched_df, tmdb_api_key, "Film", "Year")
                    st.dataframe(watched_df, use_container_width=True, hide_index=True)
                else:
                    st.caption("No watch records for this period.")
            else:
                st.caption("No watched data loaded.")
except Exception:
    pass
...
st.caption(f"{period_label} — all records")

rank_tab1, rank_tab2, rank_tab3 = st.tabs(["🎤 Artists", "💿 Albums", "🎵 Tracks"])
...
with rank_tab1:
    ...
    if not df_artists.empty:
        ...
        df_artists = df_artists.rename(columns={
            ...
            "minutes": "Minutes", "unique_tracks": "Unique Tracks",
        })
...
with rank_tab2:
    ...
    if not df_albums.empty:
        ...
        df_albums = df_albums.rename(columns={
            ...
            "plays": "Plays", "minutes": "Minutes", "unique_tracks": "Unique Tracks",
        })
...
with rank_tab3:
    ...
    if not df_tracks.empty:
        ...
        df_tracks = df_tracks.rename(columns={
            ...
            "album": "Album", "plays": "Plays", "minutes": "Minutes",
        })
